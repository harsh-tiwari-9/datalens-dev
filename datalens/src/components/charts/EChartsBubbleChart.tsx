...
interface EChartsBubbleChartProps extends ChartProps {
  showValues?: boolean
  showLabels?: boolean
  maxBubbles?: number
...
export const EChartsBubbleChart: React.FC<EChartsBubbleChartProps> = ({
  data,
  config = {},
  className = '',
  showValues = true,
  ...
  const optimizedData = useMemo(() => {
    if (values.length > maxBubbles) {
      console.warn(`Large dataset detected (${values.length} bubbles). Limiting to ${maxBubbles} bubbles for performance.`)
      
      // Sample data for large datasets
      const step = Math.ceil(values.length / maxBubbles)
  ...
  const bubbleData = useMemo(() => {
    return optimizedData.labels.map((label, index) => {
      ...
      const size = Math.max(sizeRange[0], Math.min(sizeRange[1], (value / stats.maxValue) * sizeRange[1]))
  ...
  }, [optimizedData, stats.maxValue, sizeRange])
        ...
        formatter: (params: any) => {
          ...
          return `
            <div style="padding: 8px;">
              ...
              <div style="color: #4f46e5; font-size: 18px; font-weight: bold;">${y.toLocaleString()}</div>
              <div style="color: #666; font-size: 12px; margin-top: 4px;">
                Index: #${x + 1} | Size: ${size.toFixed(1)}
              </div>
            </div>
          `
        }
  ...
  }, [bubbleData, title, subtitle, theme])
