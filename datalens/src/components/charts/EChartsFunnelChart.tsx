...
interface EChartsFunnelChartProps extends ChartProps {
  showValues?: boolean
  showLabels?: boolean
  maxSteps?: number
}
...
export const EChartsFunnelChart: React.FC<EChartsFunnelChartProps> = ({
  ...
  const optimizedData = useMemo(() => {
    if (values.length > maxSteps) {
      console.warn(`Large dataset detected (${values.length} steps). Limiting to ${maxSteps} steps for performance.`)
  ...
  }, [funnelData, title, subtitle, theme, showLabels, showValues, stats.maxValue, stats.total])
    ...
    <div className={`w-full h-full ${className}`}>
      ...
      <div className="grid grid-cols-4 gap-4 mb-6">
        <div className="bg-gradient-to-r from-indigo-50 to-indigo-100 dark:from-indigo-900/20 dark:to-indigo-800/20 rounded-lg p-4">
          ...
          <div className="text-xs text-slate-500">Top Step</div>
        </div>
        <div className="bg-gradient-to-r from-emerald-50 to-emerald-100 dark:from-emerald-900/20 dark:to-emerald-800/20 rounded-lg p-4">
          ...
          <div className="text-xs text-slate-500">Bottom Step</div>
        </div>
